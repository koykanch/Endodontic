...
$timedemo = substr($timedemo1, 0,8);
$image = @addslashes(file_get_contents($_FILES['f1']['tmp_name']));

if($_POST['HNpatient'] == null || $_POST['codestudent'] == null || $_POST['dentname'] == null || $_POST['endo_time'] == null ){
	?>
		<script>
			window.alert('HN or Student code or Dent code or Date of treatement is empty');
			window.location = "endorecord.php";
		</script>
	<?php
}else{

$duplicate = "SELECT * FROM dent_hardcopy WHERE HN = '$HN' AND hard_Date = '$datedemo'"."'$timedemo' AND student_code = '$stu' AND dentId = '$dent'";


$resduplicate = $conn->query($duplicate);
$objduplicate = mysqli_fetch_array($resduplicate);
	
	if(isset($objduplicate['HN'])){
		?>
		<script>
			window.alert('Treatment history is duplicate');
			window.location = "endorecord.php";
		</script>
	<?php
	}else{
